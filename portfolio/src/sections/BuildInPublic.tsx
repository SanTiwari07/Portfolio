  ...
  return (
    <div className="w-full overflow-hidden">
      <motion.img
        ...
        alt={`${GITHUB_USER}'s GitHub contribution graph showing commit activity`}
...
const LiveSystems: React.FC = () => {
  ...
  const [repos, setRepos] = useState<Repo[]>(featuredRepos);
  const [githubStats, setGithubStats] = useState({ repos: '20+', stars: '0' });
  ...
  useEffect(() => {
    ...
    fetch(`https://api.github.com/users/${GITHUB_USER}/repos?per_page=100`, {
      ...
      .then((data: unknown[]) => {
        ...
        const totalStars = data.reduce((acc: number, repo: unknown) => {
          const r = repo as { stargazers_count: number };
          return acc + (r.stargazers_count || 0);
        }, 0);
  ...
  return (
    <section ref={ref} id="github" className="relative w-full overflow-hidden bg-background py-28 md:py-40">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-[35%] pointer-events-none hidden lg:block z-0" aria-hidden="true">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 md:px-12 lg:px-16">
        ...
        >
          ...
          <span className="text-[10px] tracking-[0.5em] uppercase text-text-secondary font-medium">07 / Live Systems</span>
        </motion.div>

        <div className="overflow-hidden mb-4">
          <VelocityScroll intensity={0.5}>
            <motion.h2
              className="text-[clamp(44px,6vw,96px)] font-black tracking-[-0.03em] text-text-primary leading-[0.9] uppercase"
              initial={{ y: '100%' }}
        ...
        <motion.p
          className="text-text-secondary text-base max-w-md mb-16"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          <div className="lg:col-span-5 space-y-12">
            {/* Stats */}
            <div className="grid grid-cols-3 gap-0 border border-border-light" role="list" aria-label="GitHub statistics">
              {dynamicMetrics.map((m, i) => (
                <motion.div
                  key={i}
                  className="p-6 text-center border-r border-border-light last:border-r-0"
                  initial={{ opacity: 0, y: 20 }}
                  animate={isInView ? { opacity: 1, y: 0 } : {}}
            ...
            {/* Contribution Graph */}
            <motion.div
              className="p-8 border border-border-light"
              initial={{ opacity: 0, y: 20 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: 0.4, duration: 0.8 }}
            >
              <div className="flex items-center gap-3 mb-6">
                <div className="w-1.5 h-1.5 rounded-full bg-primary pulse-dot" aria-hidden="true" />
                <span className="text-[10px] tracking-[0.4em] uppercase text-text-secondary font-medium">Contribution Activity</span>
              </div>
            ...
            <motion.a
              ...
              className="group inline-flex items-center gap-4 text-xs tracking-[0.25em] uppercase font-semibold text-text-primary hover:text-primary transition-colors duration-300 focus-visible:outline-2 focus-visible:outline-primary"
              initial={{ opacity: 0 }}
              animate={isInView ? { opacity: 1 } : {}}
              transition={{ delay: 0.6 }}
              data-cursor="hover"
              aria-label={`Visit ${GITHUB_USER} on GitHub`}
            >
              ...
              @{GITHUB_USER}
              <span className="w-8 h-[1px] bg-current group-hover:w-14 transition-all duration-300" aria-hidden="true" />
            </motion.a>
          </div>

          {/* Repo List */}
          <div className="lg:col-span-7 space-y-0" role="list" aria-label="Top GitHub repositories">
            {repos.map((repo, i) => (
              <motion.a
                key={repo.name}
                href={repo.url}
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-start justify-between gap-8 border-b border-border-light py-8 hover:border-primary/30 transition-colors duration-300"
                initial={{ opacity: 0, x: 30 }}
                animate={isInView ? { opacity: 1, x: 0 } : {}}
                transition={{ delay: 0.2 + i * 0.1, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
                data-cursor="hover"
                role="listitem"
                aria-label={`${repo.name} — ${repo.description} (${repo.stars} stars)`}
              >
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="text-base font-black uppercase tracking-tight text-text-primary group-hover:text-primary transition-colors duration-300">
                      {repo.name}
                    </h3>
                    <span
                  ...
                  <p className="text-xs text-text-secondary leading-relaxed mb-3">{repo.description}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {repo.topics.map((t) => (
                      <span key={t} className="text-[8px] tracking-[0.2em] uppercase px-2 py-0.5 border border-border-light text-text-secondary">
                        {t}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex-shrink-0 flex flex-col items-end gap-2">
                  <div className="flex items-center gap-1 text-text-secondary" aria-label={`${repo.stars} stars`}>
                    <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87L18.18 21 12 17.77 5.82 21 7 14.14 2 9.27l6.91-1.01L12 2z"/>
                    </svg>
                    <span className="text-xs font-mono">{repo.stars}</span>
                  </div>
                  <div className="w-6 h-[1px] bg-border-light group-hover:w-10 group-hover:bg-primary transition-all duration-300" aria-hidden="true" />
                </div>
              </motion.a>
            ))}
          </div>
