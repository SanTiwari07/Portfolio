...
import { BoundingBox, Ramp, Track, Vehicle, Goal, Train, Heightmap, RailwayCollider } from './models'
import { angularVelocity, levelLayer, position, rotation, useStore } from './store'
import { Checkpoint, Clock, Speed, Minimap, Intro, Help, Editor, Finished, Recovery } from './ui'
  ...
  return (
    <Intro>
      <Canvas key={`${dpr}${shadows}`} dpr={[1, dpr]} shadows={shadows} camera={{ position: [0, 5, 15], fov: 50 }}>
        ...
        <Physics allowSleep={false} broadphase="SAP" defaultContactMaterial={{ contactEquationRelaxation: 10, friction: 0.5, restitution: 0.0 }}>
        ...
        <Environment files="/game/textures/dikhololo_night_1k.hdr" />
        <ToggledMap />
